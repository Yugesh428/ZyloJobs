...
interface Onboarding { id: string; status: string; joiningDate: string; salaryAmount: number; salaryPeriod: string; worker?: { fullName: string; email: string }; company?: { companyName: string }; job?: { jobRole: string; department: string }; }
...
export default function CompanyWorkersPage() {
  ...
  return (
    <ReportShell
      ...
      description="Workers placed with each company via onboarding"
      stats={[
        { label: "Total Placements", value: total },
        { label: "Active",           value: count("active") },
        { label: "Offer Sent",       value: count("offer_sent") },
        { label: "Terminated",       value: count("terminated") },
      ]}
    >
      <DataTable data={data} columns={columns} emptyMessage="No placements found" />
    </ReportShell>
  );
}
